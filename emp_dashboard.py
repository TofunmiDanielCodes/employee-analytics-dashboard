...
<style>
    ...
    label, .stMultiSelect label, .stSlider label, .stSelectSlider label, .stSelectbox label {
        color: #C8D0DC !important; font-weight: 600 !important;
    }
</style>
...
def theme(fig, height=400, showlegend=None, legend_h=False, xangle=None,
          yaxis2=None, hide_colorscale=False):
    """Apply dark theme. Title font MUST be set separately via title_font_color."""
    kw = dict(
        paper_bgcolor=BG,
        plot_bgcolor=BG,
...
# ── Sidebar ──────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("<h2 style='color:#FFFFFF;font-weight:800;margin-bottom:4px'>🏢 Employee Analytics</h2>", unsafe_allow_html=True)
    st.markdown("---")
    st.markdown("<p style='color:#C8D0DC;font-size:15px;font-weight:700;margin-bottom:8px'>🎚️ Filters</p>", unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown(
        "<p style='color:#C8D0DC;font-size:13px;font-weight:600'>1,000 employees · 12 departments</p>",
        unsafe_allow_html=True
    )
...
ret_pct    = ret_risk / len(df) * 100 if len(df) else 0
male_avg   = df[df["gender_group"] == "Male"]["salary"].mean()
female_avg = df[df["gender_group"] == "Female"]["salary"].mean()
pay_gap    = abs((male_avg - female_avg) / male_avg * 100) if male_avg and not np.isnan(male_avg) else 0
...
with tab1:
    ...
    with col_a:
        ...
        fig.add_vline(x=avg_sal,    line_dash="dash", line_color=AMBER,
                      annotation_text=f"Avg ${avg_sal:,.0f}", annotation_font_color=AMBER)
        fig.add_vline(x=median_sal, line_dash="dot",  line_color=GREEN,
                      annotation_text=f"Median ${median_sal:,.0f}", annotation_font_color=GREEN)
    ...
    with col_c:
        ds = dept_stats.sort_values("Avg_Salary")
        fig3 = px.bar(ds, x="Avg_Salary", y="department",
                      color="Avg_Salary",
                      color_continuous_scale=[[0,"#2C3E70"],[0.5,BLUE],[1,TEAL]],
                      title="Average Salary by Department",
                      labels={"Avg_Salary":"Avg Salary ($)","department":""},
                      text=ds["Avg_Salary"].apply(lambda v: f"${v:,.0f}"))
        fig3.update_traces(textposition="outside")
        fig3.add_vline(x=avg_sal, line_dash="dash", line_color=AMBER,
                       annotation_text="Company Avg", annotation_font_color=AMBER)
        theme(fig3, height=420, hide_colorscale=True)
    ...
    with col_d:
        fig4 = px.scatter(dept_stats, x="Headcount", y="Avg_Salary",
                          ...
                          title="Headcount vs Avg Salary (bubble = total payroll)",
    ...
    tier_df["pct"] = (
        tier_df["count"] /
        tier_df.groupby("department")["count"].transform("sum") * 100
    ).round(1)
...
with tab2:
    ...
    with col_a:
        fig = px.bar(gender_sal, x="Gender", y="Avg Salary",
        ...
        fig.add_hline(y=avg_sal, line_dash="dash", line_color=RED,
                      annotation_text="Company Avg", annotation_font_color=RED)
    ...
    gd["pct"] = (
        gd["count"] / gd.groupby("department")["count"].transform("sum") * 100
    ).round(1)
    dept_f_order = (
        gd[gd["gender_group"] == "Female"]
        .groupby("department")["pct"].sum()
        .sort_values().index.tolist()
    )
    gd["label"] = gd["pct"].apply(lambda v: f"{v:.0f}%" if v >= 8 else "")
    ...
    fig4.add_vline(x=50, line_dash="dash", line_color=RED,
                   annotation_text="50% Parity", annotation_font_color=RED)
    ...
    mf = df[df["gender_group"].isin(["Male","Female"])]
    pivot = mf.groupby(["department","gender_group"])["salary"].mean().unstack()
    ...
    with col_e:
        ...
        fig5.update_layout(
            ...
            title_font_color=WHITE, title_font_size=14,
            xaxis=dict(gridcolor=GRID, tickangle=-30, color=TEXT),
    ...
    with col_f:
        ...
        display_pivot = pivot[["department","Male","Female","Pay Gap %"]].copy()
        display_pivot.columns = ["Department","Male Avg $","Female Avg $","Pay Gap %"]
        st.dataframe(
...
# ══════════════════════════════════════════════════════
with tab3:
    st.markdown("<div class='section-header'>Age Profile</div>", unsafe_allow_html=True)
    ...
    with col_a:
        ...
        fig.add_vline(x=float(df["age"].mean()), line_dash="dash", line_color=AMBER,
                      annotation_text=f"Avg {df['age'].mean():.1f}", annotation_font_color=AMBER)
        fig.add_vline(x=55, line_dash="dot", line_color=RED,
                      annotation_text="Retirement Zone", annotation_font_color=RED)
    ...
    with col_b:
        ...
        theme(fig2, height=340, hide_colorscale=True)
        st.plotly_chart(fig2, use_container_width=True)

    st.markdown("---")
    st.markdown("<div class='section-header'>Tenure & Loyalty</div>", unsafe_allow_html=True)
    ...
    with col_c:
        ...
        p = np.poly1d(z)
        x_line = np.linspace(float(df["tenure_years"].min()), float(df["tenure_years"].max()), 100)
    ...
    with col_d:
        ...
        fig4.update_layout(
            ...
            yaxis=dict(title="Avg Salary ($)", gridcolor=GRID, color=TEXT),
            yaxis2=dict(title="Headcount", overlaying="y", side="right",
                        showgrid=False, color=TEXT),
            legend=dict(bgcolor="rgba(0,0,0,0)", orientation="h",
                        yanchor="bottom", y=1.02, font=dict(color=TEXT)),
            margin=dict(t=55, b=30, l=10, r=10),
            height=380,
        )
        st.plotly_chart(fig4, use_container_width=True)
    ...
    fig5 = px.treemap(dept_head,
                      ...
                      color_discrete_map=DEPT_COLORS,
                      title="Headcount — Department → Gender")
...
with tab4:
    ...
    with col_a:
        ...
        fig.update_layout(
            ...
            title_font_color=WHITE, title_font_size=14,
            xaxis=dict(gridcolor=GRID, dtick=1, color=TEXT),
            yaxis=dict(title="Number of Hires", gridcolor=GRID, color=TEXT),
            yaxis2=dict(title="YoY Change (%)", overlaying="y", side="right",
                        showgrid=False, zeroline=True, zerolinecolor=RED, color=TEXT),
            legend=dict(bgcolor="rgba(0,0,0,0)", orientation="h",
                        yanchor="bottom", y=1.02, font=dict(color=TEXT)),
            margin=dict(t=55, b=30, l=10, r=10),
            height=380,
    ...
    with col_b:
        ...
        st.metric("Total Hires",      f"{int(hire_annual['hires'].sum()):,}")
...
with tab5:
    ...
    if search_name:
        exp = exp[
            exp["first_name"].str.contains(search_name, case=False, na=False) |
    ...
    if len(exp) > 0:
        ...
        with col_v1:
            ...
            theme(fig, height=300, hide_colorscale=True)
            st.plotly_chart(fig, use_container_width=True)
...
st.markdown(
    ...
    f"1,000 employees across 12 departments"
    f"</p>",
    unsafe_allow_html=True,
)
