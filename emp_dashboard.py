...
with st.sidebar:
    ...
    hire_years = sorted(df_full["hire_year"].dropna().astype(int).unique().tolist())
    year_range = st.select_slider(
        "Hire Year Range",
        options=hire_years,
        value=(min(hire_years), max(hire_years)),
    )
...
with tab1:
    ...
    with col_c:
        ...
        fig3.update_layout(**THEME, height=420, coloraxis_showscale=False)
    ...
    with col_d:
        ...
        fig4.update_layout(**THEME, height=420, showlegend=False)
    ...
    fig5.update_layout(**THEME, height=370,
...
with tab3:
    ...
    with col_b:
        ...
        fig2.update_layout(**THEME, height=340, coloraxis_showscale=False)
        st.plotly_chart(fig2, use_container_width=True)
    ...
    fig5.update_layout(**THEME, height=440)
    st.plotly_chart(fig5, use_container_width=True)
...
with tab4:
    ...
    fig3.update_layout(**THEME, height=400,
                       legend=dict(orientation="h", yanchor="bottom", y=1.02))
    st.plotly_chart(fig3, use_container_width=True)
